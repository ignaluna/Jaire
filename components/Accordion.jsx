...
const Accordion = ({ data, move, number }) => {
  ...
  return (
    <div className="w-screen accordion max-md:h-[15vh] bg-black bg-opacity-75 overflow-hidden" >
      {data.map((item, i) => (
        item.number === number ? 
        <div className="h-full w-full" key={i} onClick={() => toggle(i)}>
            <div className={` border-r-4 border-green-900 accordion max-md:h-[15vh] absolute w-screen h-full flex justify-center items-center 
            titleq ${move === "left" ? selected === i ? "show" : "" : selected === i ? "" : "show"}
            ${move === "left"? "titulos font-jura max-md:text-[200%]" : "titulos font-jura max-md:text-[200%]"}`} >
